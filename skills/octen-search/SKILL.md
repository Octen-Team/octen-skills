...
curl -s -X POST "https://api.octen.ai/search" \
  ...
  -H "X-Api-Key: ${OCTEN_API_KEY}" \
  -d '{
    "query": "best ramen shops in Tokyo",
    "count": 5,
    "country": "JP"
  }'
```
...
| `search_options` | object | No | - | Options applied to **each** sub-query — same fields and defaults as the [Parameters](#parameters) table above (`topic`, `count`, `include_domains`, `country`, time filters, `highlight`, `full_content`, `include_images`, …) |
